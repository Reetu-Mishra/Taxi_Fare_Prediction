...
distance_df = pd.read_csv("mode_distance1.csv")
...
# Creating Location Distionary & List.
...
# Get all drop locations where either (pickup → drop) or (drop → pickup) exists
filtered_df = distance_df[
    (distance_df['pickup_location'] == pickup.lower()) | 
    (distance_df['drop_location'] == pickup.lower())
]

# Extract drop options considering both directions
drop_options = set()
...
def get_distance(pickup, drop):
    ...
    if not row.empty:
        return round(float(row.iloc[0]['average_trip_distance']), 2)
    return None
